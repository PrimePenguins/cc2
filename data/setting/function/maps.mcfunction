...
execute if score $map setting matches 4.. run scoreboard players set $map setting 0
execute if score $map setting matches ..-1 run scoreboard players set $map setting 3
...
execute if score $map setting matches 3 run data merge entity @e[tag=map.text,limit=1] {text:{"translate":"setting.map.selected",with:[{"translate":"map.name.desert",color:"gold",bold:1b}]}}
